// https://github.com/basavanag/jasmine-xml2html-converter
...
var generateTDTable = function (reportXml) {
  ...
  var testDetailsTable = '<tr><th id="td-table-header">Spec Description</th> \
    ...
    <th id="td-table-header">Details</th> \
    <th id="td-table-header">Time</th> \
    <th id="td-table-header">Assertions</th></tr>';
  ...
  for (var i = 0; i < totalSuites; i++) {
    ...
    dataSeries = (i == totalSuites - 1) ? dataSeries : dataSeries + ',';
		...
		testDetailsTable += '<tr><td id="td-table-spec" colspan=5>' + suiteName + '</td></tr>';
    ...
    // Capture tescase execution details for each testsuite
		for(var j in testcases) {
          ...
          if(totalFailures > 0 && testFailed != undefined)
              testFailed = testcases[j].childNamed('failure').toString();
			      
          var testSkipped = testcases[j].childNamed('skipped');

            if(testFailed && totalFailures > 0) {            
              testFailed = testFailed.replace('<failure message="', "");
              testFailed = testFailed.replace('">', "<br><br>");
              testFailed = testFailed.replace('</failure>', "");
        ...
        testDetailsTable += '<td id="td-table-test-fail">Error</td><td>' + testError + '</td>';
            ...
            testDetailsTable += '<td>' + testcases[j].attr.time; + '</td>'
            testDetailsTable += '<td>' + testcases[j].attr.assertions; + '</td>'
